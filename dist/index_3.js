...
const getGiveaways = async () => {
    if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_3 ||
        !process.env.GOOGLE_PRIVATE_KEY_3 ||
        !process.env.GOOGLE_GIVEAWAYS_SHEET_ID_3) {
        throw new Error('GOOGLE_SERVICE_ACCOUNT_EMAIL_3, GOOGLE_PRIVATE_KEY_3 or GOOGLE_GIVEAWAYS_SHEET_ID_3 missing');
    }
    const doc = new google_spreadsheet_1.GoogleSpreadsheet(process.env.GOOGLE_GIVEAWAYS_SHEET_ID_3);
    const client_email = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_3;
    const private_key = process.env.GOOGLE_PRIVATE_KEY_3.replace(/\\n/g, '\n');
    // Authorize
...
const createQueue = async () => {
    if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_3 || !process.env.GOOGLE_PRIVATE_KEY_3 || !process.env.GOOGLE_SHEET_ID_3) {
        throw new Error('GOOGLE_SERVICE_ACCOUNT_EMAIL_3, GOOGLE_PRIVATE_KEY_3 or GOOGLE_SHEET_ID_3 missing');
    }
    const doc = new google_spreadsheet_1.GoogleSpreadsheet(process.env.GOOGLE_SHEET_ID_3);
    const client_email = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_3;
    const private_key = process.env.GOOGLE_PRIVATE_KEY_3.replace(/\\n/g, '\n');
    // Authorize
    console.log('Authorizing with Google');
...
const discordBot = async () => {
    if (!process.env.DISCORD_TOKEN_3 || !process.env.DISCORD_SERVER_ID_3 || !process.env.DISCORD_WHITELIST_ROLE_ID_3) {
        throw new Error('DISCORD_SERVER_ID_3, DISCORD_TOKEN_3 or DISCORD_WHITELIST_ROLE_ID_3 missing');
    }
    const TOKEN = process.env.DISCORD_TOKEN_3;
    const SERVER_ID = process.env.DISCORD_SERVER_ID_3;
    const WHITELIST_ROLE_ID = process.env.DISCORD_WHITELIST_ROLE_ID_3;
    ...
    client.on('messageCreate', async (msg) => {
        ...
        if (member) {
            ...
            // if (process.env.PRESALE_TOKEN_ADDRESS_3) {
            ...
            //           },
            //         });
            //         const idx = data.findIndex((account) => account.tokenAddress === process.env.PRESALE_TOKEN_ADDRESS_3);
            //         msg.reply(
            //           idx > -1
