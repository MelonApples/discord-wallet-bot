...
const getGiveaways = async () => {
    if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_2 ||
        !process.env.GOOGLE_PRIVATE_KEY_2 ||
        !process.env.GOOGLE_GIVEAWAYS_SHEET_ID_2) {
        throw new Error('GOOGLE_SERVICE_ACCOUNT_EMAIL_2, GOOGLE_PRIVATE_KEY_2 or GOOGLE_GIVEAWAYS_SHEET_ID_2 missing');
    }
    const doc = new google_spreadsheet_1.GoogleSpreadsheet(process.env.GOOGLE_GIVEAWAYS_SHEET_ID_2);
    const client_email = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_2;
    const private_key = process.env.GOOGLE_PRIVATE_KEY_2.replace(/\\n/g, '\n');
    // Authorize
...
const createQueue = async () => {
    if (!process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_2 || !process.env.GOOGLE_PRIVATE_KEY_2 || !process.env.GOOGLE_SHEET_ID_2) {
        throw new Error('GOOGLE_SERVICE_ACCOUNT_EMAIL_2, GOOGLE_PRIVATE_KEY_2 or GOOGLE_SHEET_ID_2 missing');
    }
    const doc = new google_spreadsheet_1.GoogleSpreadsheet(process.env.GOOGLE_SHEET_ID_2);
    const client_email = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL_2;
    const private_key = process.env.GOOGLE_PRIVATE_KEY_2.replace(/\\n/g, '\n');
    // Authorize
    console.log('Authorizing with Google');
...
const discordBot = async () => {
    if (!process.env.DISCORD_TOKEN_2 || !process.env.DISCORD_SERVER_ID_2 || !process.env.DISCORD_WHITELIST_ROLE_ID_2) {
        throw new Error('DISCORD_SERVER_ID_2, DISCORD_TOKEN_2 or DISCORD_WHITELIST_ROLE_ID_2 missing');
    }
    const TOKEN = process.env.DISCORD_TOKEN_2;
    const SERVER_ID = process.env.DISCORD_SERVER_ID_2;
    const WHITELIST_ROLE_ID = process.env.DISCORD_WHITELIST_ROLE_ID_2;
    ...
    client.on('messageCreate', async (msg) => {
        ...
        if (member) {
            ...
            // if (process.env.PRESALE_TOKEN_ADDRESS_2) {
            ...
            //           },
            //         });
            //         const idx = data.findIndex((account) => account.tokenAddress === process.env.PRESALE_TOKEN_ADDRESS_2);
            //         msg.reply(
            //           idx > -1
